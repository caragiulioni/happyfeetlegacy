...
class Player {
  ...
  constructor(root) {
    ...
    this.y = GAME_HEIGHT - PLAYER_HEIGHT - Math.floor(90 * SCALE);

    // We create a DOM node. We will be updating the DOM node every time we move the player, so we store a reference to the
    // DOM node in a property.
    this.domElement = document.createElement('img');
    this.domElement.id ="player"
    this.domElement.src = 'images/penguinMOVE.gif';
    this.domElement.style.width = `${Math.floor((IS_MOBILE ? 80 : 100) * SCALE)}px`;
  ...
  moveLeft() {
    if (this.x > 0) {
      ...
      //console.log("PLAYER LEFT", this.x);
    }
  ...
  moveRight() {
    if (this.x + PLAYER_WIDTH < GAME_WIDTH - PLAYER_WIDTH) {
      this.x = this.x + PLAYER_WIDTH;
    }
    this.domElement.style.left = `${this.x}px`;
  }
